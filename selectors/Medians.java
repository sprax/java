...
public class Medians
{
  ...
  public static int test_medianFromSortedRowsAndColumns(int size) 
  {
    ...
    if (zoid > 1) {
      for (int j = 0; j < ZZZ.length; j++) {
        ...
        Sx.puts("median of ZZZ[" +j+ "] is " + medianOfAA_mergeArrays(ZZZ[j]) + " merge_Nwise");
        Sx.puts("median of ZZZ[" +j+ "] is " + medianOfAA_binarySearchKth(ZZZ[j]) + " binary search");
        Sx.puts("median of ZZZ[" +j+ "] is " + medianOfAA_queueWalkKth(ZZZ[j]) + " queue walk");        
  ...
  public static int checkArray2(final int AA[][])
  {
    ...
    for (int row = 0; row < nRows; row++) {
      ...
      if (nCols < 1 || nCols != AA[0].length)
        throw new IllegalArgumentException("checkArray2: row "+row+" size "+nCols+"!="+AA[0].length);
    }
    return nCols;
  }
  ...
  public static int test_timeMedianOfRowColSortedArrays(int level)
  {
    ...
    for (int k = 0; k < q; k++) {
      switch(k) {
      case 0:
        System.out.format("\n medianOfAA_binSearchKth    (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmpk[j]);
        }    
        break;
      case 1:
        System.out.format("\n medianOfAA_mergeArrays     (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmar[j]);
        }
        break;
      case 2:
        System.out.format("\n medianOfAA_mergeArraysPq   (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmpq[j]);
        }
        break;
      case 3:
        System.out.format("\n medianOfAA_mergeArraysPqSl (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmps[j]);
        }
        break;
      case 4:
        System.out.format("\n medianOfAA_queueWalkKth    (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmpw[j]);
        }    
        break;
      case 5:
        System.out.format("\n medianOfAA_mergeArraysPairs(%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
          System.out.format("%7d ", tmap[j]);
        }
        break;
      case 6:
        System.out.format("\n medianOfAA_youngify        (%d, %d) minVal %d  maxInc %d -> %d  t: ", nRows, nCols, minVal, maxInc, ms[k]);
        for (int j = 0; j < times; j++) {
   ...
   */
  public static int limitCountNotGreaterThanVal(int AA[][], int nRows, int nCols, int val, int maxCount)
  {
    int countNGT = 0;
    for (int row = nRows-1, col = 0; row >= 0 && col < nCols; ) {
      if (AA[row][col] <= val) {
  ...
  {
    return nRows*nCols - countLessThan(AA, nRows, nCols, val) - countGreaterThan(AA, nRows, nCols, val);
  }
   ...
   */
  public static int binarySearchForKthLargest_recursive(int AA[][], int nRows, int nCols, int first, int last, int k)
  {
    if(first > last) {
  ...
  public static int binarySearchForKthSmallest_recursive_inet(int AA[][], int nRows, int nCols, int first, int last, int k)
  {
    if(first > last) {
    ...
    int mid = (first + last) >> 1;       // average of first and last values
      ...
      if (x == k)
        return Math.min(mid, binarySearchForKthSmallest_recursive_inet(AA, nRows, nCols, first, mid-1, k));
      else if (x > k) 
  ...
  public static int test_binarySearchForKthLargest_recursive(int AA[][], int minOrdinal, int maxOrdinal)
  {
    if (AA == null) {
      int sqr = 7;
    ...
    for (int k = first; k <= last; k++) {
      int kth = binarySearchForKthSmallest_recursive_inet(AA, nRows, nCols, minVal, maxVal, size+1 - k);
      System.out.format("test_bsKth_inet[%d .. %d]    %dth / %d is %d\n", minVal, maxVal, k, size, kth);
    }
    Sx.puts();
    for (int k = first; k <= last; k++) {
  ...
  public static int medianOfAA_binarySearchKth(final int AA[][])
  {
    ...
    int ord = (size+1)/2;
    if (oddSize) {
      return binarySearchForKthLargest_recursive(AA, nRows, nCols, AA[0][0], AA[nRows-1][nCols-1], ord);
    } else {
      ...
      int kth = binarySearchForKthLargest_recursive(AA, nRows, nCols, AA[0][0], AA[nRows-1][nCols-1], ord);
      int kp1 = binarySearchForKthLargest_recursive(AA, nRows, nCols, AA[0][0], AA[nRows-1][nCols-1], ord+1);
      return (kth + kp1)/2;
    }
  }
