...
public class ConnectedComponents<V extends Vertex>
{
    /*************************************************************************
    Compilation: javac CC.java Execution: java CC filename.txt Dependencies:
    Graph.java
    ...
    *************************************************************************/
    ...
    components in an
    undirected graph.  The <em>id</em> operation determines in which connected
    component a given
    vertex lies; the <em>connected</em> operation determines whether two
    vertices are in the same
    connected component; the <em>count</em> operation determines the number of
    connected
    components; and the <em>size</em> operation determines the number of
    vertices in the connect
    component containing a given vertex.

    The <em>component identifier</em> of a connected component is one of the
    vertices in the
    connected component: two vertices have the same component identifier if and
    only if they are
    ...
    <p>
    For additional documentation, see <a href="/algs4/41graph">Section 4.1</a>
    of <i>Algorithms,
    ...
    */
    ...
    {
        int numVerts = graph.getNumVerts();
    ...
    */
    ...
    {
        /*************************
         * Graph G = new Graph(in); ConnectedComponents<T> cc = new
         ConnectedComponents<Vertex>(G);
         *
         * // number of connected components int M = cc.count(); Sx.puts(M + "
         components");
         *
         * // compute list of vertices in each connected component
         Queue<Integer>[] components =
         * (Queue<Integer>[]) new Queue[M]; for (int i = 0; i < M; i++) {
         components[i] = new
         * Queue<Integer>(); } for (int v = 0; v < G.V(); v++) {
         components[cc.id(v)].enqueue(v); }
         *
         * // print results for (int i = 0; i < M; i++) { for (int v :
         components[i]) {
         * Sx.print(v + " "); } Sx.puts(); }
        ****************************/
    }
